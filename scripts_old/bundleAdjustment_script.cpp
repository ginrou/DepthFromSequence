...
int main(int argc, char* argv[]) {
  ...
  vector<Point3f> pt_vec = gen_3d_points(20, cv::Point3f(-10,-10,10), cv::Point3f(10,10,30));
  vector<Point3f> cam_t_vec = gen_3d_cam_t(3);
  vector<Point3f> cam_rot_vec = gen_3d_cam_rot(3);

  vector<Point2f> projected = project_3d_to_2d(cam_t_vec[0], cam_rot_vec[0], pt_vec);

  for(int i = 0; i < projected.size(); ++i ) {
    Point2f pt = projected[i];
    printf("%lf\t%lf\n", pt.x, pt.y);
  }
